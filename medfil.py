# -*- coding: utf-8 -*-
"""
Created on Mon Oct  7 01:59:01 2019
...
if __name__ == '__main__':
    img = cv2.imread('C:\\Users\\madhu\\Documents\\books n notes\\vit sem1\\set1\\lenasaltpep.PNG')
    
    blur = cv2.medianBlur(img,5)

    plt.subplot(121),plt.imshow(img),plt.title('Noisy Image')
    plt.xticks([]), plt.yticks([])
    plt.subplot(122),plt.imshow(blur),plt.title('After Median Filter')
    plt.xticks([]), plt.yticks([])
    plt.show()
    m=mse(img, blur)
    print(m)
